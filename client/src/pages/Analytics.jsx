...
const BAR_COLORS = ["#7c3aed","#10b981","#f59e0b","#ef4444","#a78bfa","#60a5fa","#34d399","#fb923c"];
...
export default function Analytics() {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#06060f", position: "relative" }}>
      ...
      <div style={{ position: "fixed", top: "5%", right: "10%", width: "500px", height: "500px", background: "radial-gradient(circle, rgba(124,58,237,0.06) 0%, transparent 70%)", pointerEvents: "none", zIndex: 0 }} />
        ...
        <main style={{ flex: 1, padding: "28px 32px", overflowY: "auto" }}>
          {loading ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "16px" }}>
                <StatCard title="TOTAL EVENTS"    value={engagement?.totalEvents}    icon="◈" color="#7c3aed" delay="delay-1" />
              ...
              <div className="animate-fade-up delay-2" style={{ ...glassPanel, padding: "28px" }}>
                <div style={{ marginBottom: "24px" }}>
                  ...
                  <p style={{ fontFamily: "sans-serif", fontSize: "22px", letterSpacing: "2px", color: "var(--text)", margin: 0 ,fontWeight:"bold"}}>
                ...
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={engagement?.events} barGap={4} barCategoryGap="30%">
                    ...
                    <XAxis dataKey="name" tick={{ fontSize: 11, fill: "rgba(255,255,255,0.25)", fontFamily: "sans-serif" }} axisLine={false} tickLine={false} />
                    <YAxis tick={{ fontSize: 11, fill: "rgba(255,255,255,0.25)", fontFamily:"sans-serif" }} axisLine={false} tickLine={false} />
                    <Tooltip
                      content={<CustomTooltip />}
                      cursor={{ fill: "transparent" }}
                    />
                    <Bar dataKey="attendees" name="Attendees" radius={[6,6,0,0]}>
                      {engagement?.events?.map((_, i) => <Cell key={i} fill={BAR_COLORS[i % BAR_COLORS.length]} />)}
                    </Bar>
                    <Bar dataKey="capacity" name="Capacity" radius={[6,6,0,0]} fill="rgba(255,255,255,0.06)" />
                  </BarChart>
                </ResponsiveContainer>
                <div style={{ display: "flex", gap: "20px", marginTop: "16px", paddingTop: "16px", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                  {[{ label: "Attendees", color: "#7c3aed" }, { label: "Capacity", color: "rgba(255,255,255,0.1)" }].map(l => (
                    <div key={l.label} style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                      <div style={{ width: "10px", height: "10px", borderRadius: "2px", background: l.color, border: l.label === "Capacity" ? "1px solid rgba(255,255,255,0.15)" : "none" }} />
              ...
              <div className="animate-fade-up delay-3" style={{ ...glassPanel, overflow: "hidden" }}>
                <div style={{ padding: "20px 24px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                  <p style={{ fontFamily:"sans-serif", fontSize: "9px", color: "rgba(255,255,255,0.3)", letterSpacing: "2px", marginBottom: "6px" }}>BREAKDOWN</p>
                  <p style={{ fontFamily: "sans-serif", fontSize: "22px", letterSpacing: "2px", color: "var(--text)", margin: 0 ,textShadow: "0 0 20px rgba(255, 255, 255, 0.2), 0 0 40px rgba(37, 99, 235, 0.3)",fontWeight:"bold"}}>Event Details</p>
                </div>
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead>
                    <tr style={{ background: "rgba(255,255,255,0.02)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                      {["Event","Attendees","Capacity","Fill Rate","Status"].map(h => (
                        <th key={h} style={{ padding: "12px 20px", textAlign: "left", fontFamily: "sans-serif", fontSize: "12px", fontWeight: 500, color: "rgba(255,255,255,0.3)", letterSpacing: "2px" }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                        ...
                        <tr key={ev.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)", transition: "background 0.15s" }}
                          ...
                          <td style={{ padding: "14px 20px" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                              <div style={{ width: "8px", height: "8px", borderRadius: "50%", background: BAR_COLORS[i % BAR_COLORS.length], flexShrink: 0, boxShadow: `0 0 8px ${BAR_COLORS[i % BAR_COLORS.length]}80` }} />
                          ...
                          <td style={{ padding: "14px 20px" }}>
                            <span style={{ padding: "3px 10px", borderRadius: "20px", fontFamily: "'Bebas Neue', cursive", fontSize: "9px", fontWeight: 500, letterSpacing: "1px", background: s.bg, border: `1px solid ${s.border}`, color: s.color }}>{ev.status?.toUpperCase()}</span>
                          </td>
                        </tr>
                      );
                    })}
